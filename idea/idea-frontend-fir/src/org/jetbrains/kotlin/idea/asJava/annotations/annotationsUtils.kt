...
internal fun KtAnnotatedSymbol.computeAnnotations(
    ...
    val nullabilityAnnotation = when (nullability) {
        NullabilityType.NotNull -> "org.jetbrains.annotations.NotNull"
        NullabilityType.Nullable -> "org.jetbrains.annotations.Nullable"
        else -> null
    }?.let {
        FirLightSimpleAnnotation(it, parent)
    }

    if (annotations.isEmpty()) {
        return listOfNotNull(nullabilityAnnotation)
    }
