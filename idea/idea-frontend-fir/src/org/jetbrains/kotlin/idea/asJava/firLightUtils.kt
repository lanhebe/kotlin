...
internal fun FirMemberDeclaration.computeModalityForMethod(isTopLevel: Boolean): Set<String> {
    require(this !is FirConstructor)
    return computeSimpleModality()
        .add(PsiModifier.NATIVE, isExternal)
        .add(PsiModifier.STATIC, isTopLevel)
}
...
//        if (!subtypeOfNullableSuperType) return NullabilityType.NotNull
    ...
    if (coneType is ConeClassErrorType) return NullabilityType.Unknown
    if (coneType.typeArguments.any { it is ConeClassErrorType }) return NullabilityType.Unknown
    if (coneType.classId?.shortClassName?.asString() == SpecialNames.ANONYMOUS) return NullabilityType.Unknown
    ...
    if (canonicalSignature == "[L<error>;") return NullabilityType.Unknown
